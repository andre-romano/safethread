...
from safethread.thread.datatype import SafeBase


class TestSafeBaseObj(unittest.TestCase):
    def setUp(self):
        """Initialize test objects before each test."""
        self.obj = SafeBase(10)
        self.obj_float1 = SafeBase(10.2)
        self.obj_float2 = SafeBase(10.7)

        self.obj_bool = SafeBase(True)

        self.obj_list = SafeBase([1, 2, 3, 4])
        self.obj_set = SafeBase({1, 2, 3, 4})
        self.obj_dict = SafeBase({
            'a': 'A',
            'b': 'B',
            'c': 'C',
            'd': 'D'
    ...
    def test_bool(self):
        ...
        self.assertFalse(bool(SafeBase(0)))
