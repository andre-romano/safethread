...
from safethread.thread.datatype import SafeSetThread


class TestSafeSetThread(unittest.TestCase):

    def setUp(self):
        """Create a SafeSetThread instance for testing."""
        self.safe_set = SafeSetThread([1, 2, 3])

    def test_initialization(self):
        safe_set = SafeSetThread([5, 6])
        self.assertIn(5, safe_set)

        set_custom = set([7, 8])
        safe_set = SafeSetThread(set_custom)
