...
class TestPipelineStage(unittest.TestCase):
    ...
    def test_invalid_initialization(self):
        ...
        with self.assertRaises(ThreadBase.CallableException) as context:
            PipelineStageThread(None)  # type: ignore
        with self.assertRaises(ValueError) as context:
            PipelineStageThread(lambda x: x + 3, n_threads=0)  # type: ignore

    def test_put_and_get(self):
        """
        Test that data can be put into the pipeline, processed, and retrieved correctly.
        """
        pipeline = PipelineStageThread(lambda x: x * 2)
        test_input = 5
    ...
    def test_concurrent_processing(self):
        ...
        """
        # start the pipeline
        pipeline = PipelineStageThread(lambda x: x + 1, n_threads=5)
        pipeline.start()
    ...
    def test_stop_join(self):
        ...
        pipeline = PipelineStageThread(multiply_by_two)
        pipeline.start()
        ...
        with self.assertRaises(PipelineStageThread.StoppedException) as context:
            pipeline.put(20)
    ...
    def test_pipeline_stopped_exception(self):
        ...
        pipeline = PipelineStageThread(lambda: None)
        pipeline.start()
        pipeline.stop()
        with self.assertRaises(PipelineStageThread.StoppedException):
            pipeline.put(5)
        with self.assertRaises(PipelineStageThread.StoppedException):
            pipeline.get()
    ...
    def test_connect_output(self):
        ...
        pipe1 = PipelineStageThread(lambda x: x+1)
        pipe2 = PipelineStageThread(lambda x: x*2)
