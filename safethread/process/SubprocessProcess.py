...
from .. import AbstractPicklable

from . import BaseProcess


def _run_subprocess(
        command: ListProxy[str],
        on_finish: Callable,
        timeout: float | None,
        cwd: str | None,
        env: dict | None,
    ...
    """
    cmd = list(command)
    result: SubprocessProcess.Finished
    try:
...
class SubprocessProcess(BaseProcess):
    """
    A process-safe class for running subprocess commands.

    This class inherits from BaseProcess and provides functionality to run a command in a subprocess, capture its output, and handle its completion through a callback.
    """
    ...
    def __init__(self,
        ...
        # check command
        cmd: ListProxy = multiprocessing.Manager().list()
