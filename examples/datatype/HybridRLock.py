...
from safethread.datatype import HybridRLock
...
def process_task(lock: HybridRLock, results: ListProxy, i: int):
    with lock:
        results.append(i)
...
def run_multiprocessing_example():
    ...
    processes = [multiprocessing.Process(
        target=process_task, args=(lock, results, i)) for i in range(10)]
